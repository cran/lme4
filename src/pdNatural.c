 ...
 * 
 * @brief  C functions called by methods for the pdNatural class
 * 
 */
...
SEXP
pdNatural_pdmatrix(const SEXPREC* x)
{
...
SEXP
pdNatural_corrmatrix(const SEXPREC* x)
{
    SEXP param = GET_SLOT((SEXP) x, install("param"));
    int q = asInteger(GET_SLOT((SEXP) x, install("Ncol")));
 ...
 */
SEXP
pdNatural_LMEgradient(const SEXPREC* x, const SEXPREC* Ain,
                      const SEXPREC* nlev)
{
    SEXP param = GET_SLOT((SEXP) x, install("param"));
