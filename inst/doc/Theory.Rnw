...
\citep[\S2.3]{mccullagh89:_gener_linear_model} for details).  For
...
\end{equation}
The quantity $k(\bm y,\sigma^2)$ is the \emph{normalizing factor}
defined so that the $\int_{\bm y}f_{\bm Y|\bm B}(\bm y|\bm
b,\bm\beta,\sigma^2)\,d\bm y=1$.
...
element of $\bm y$ and the corresponding element of $\bm\mu_{\bm Y|\bm
  B}$.  Consequently, the discrepancy can be written as a sum of $n$
terms that also are evaluated component-wise on $\bm\mu_{\bm Y|\bm
  B}$ and $\bm y$.
...
y|\bm b,\bm\beta)= k(\bm y)e^{-d(\bm\mu_{\bm Y|\bm B}(\bm
  b,\bm\beta)),\bm y)/2}$ and the normalization factor is $k(\bm
y)$.
...
\begin{equation}
  ...
  f_{\bm U}(\bm u|\sigma^2)=(2\pi\sigma^2)^{-q/2}e^{-\bm u\trans\bm
    u/(2\sigma^2)} .
\end{equation}
(When a generalized linear mixed model does not include the variance
scale factor, $\sigma^2$, the distribution of $\bm U$ is the standard
$q$-variate Gaussian distribution $\mathcal{N}\left(\bm 0,\bm
  I_q\right)$ with density $f_{\bm U}(\bm u)= (2\pi)^{-q/2}e^{-\bm
  u\trans\bm u/2}$.)  The random effects $\bm U$ are ``orthogonal'' in
the sense of being uncorrelated.

We note that (\ref{eq:TSPu}) provides the desired distribution $\bm
B\sim\mathcal{N}(\bm{0},\sigma^2\bm\Sigma)$ because $\bm B$, as a
linear transformation of $\bm B$, has a multivariate Gaussian
...
\end{equation}
with conditional mean $\bm\mu_{\bm Y|\bm B}(\bm b,\bm\beta)=\bm Z\bm
...
(\ref{eq:lmmCholeskyFactor}), (\ref{eq:lmmRVX}) and (\ref{eq:lmmrVy}),
...
$\bm{u}^{(i)}$, the value of the $\bm u$ at the $i$th iteration, the
...
\begin{equation}
  ...
  \begin{aligned}
    \bm\delta^{(i)}
    &=\arg\min_{\bm\delta}\left\|\bm\mu^{(i)}+\bm{M}^{(i)}
      \bm P\trans\bm\delta -\bm y\right\|^2+
    (\bm\delta+\bm u^{(i)})\trans(\bm\delta+\bm u^{(i)})\\
...
factor.  We evaluate the penalized discrepancy at $\bm u=\bm
u^{-1}+h\bm\delta^{(i)}$ for successively smaller values of $h$ until
we obtain a decrease in the penalized discrepancy or the minimum step
factor is reached.  Generally a simple strategy such as setting $h=1$
at the beginning of each iteration and successively halving $h$ if
...
orthogonal components of the residual vector, evaluated at the current
value of $\bm u$, $\bm\beta$ and $\bm\theta$.  This ``relative
offset'' or orthogonality convergence criterion is
\begin{displaymath}
  \frac{\|{\bm L^{(i)}}\trans\bm\delta^{(i)}\|/\sqrt{q}}
  {\|\bm P{\bm M^{(i)}}\trans \left(\bm y-\bm\mu^{(i)}\right)-\bm u^{(i)}\|/\sqrt{n-q}}
\end{displaymath}
...
criterion depends only not the current position and not on the path
...
\begin{equation}
  ...
  \bm\mu(\bm u,\bm\beta,\bm\theta)=\bm\mu_{\bm{0}}(\bm\beta)+\bm{M}(\bm\beta,\bm\theta,\bm y)\bm u
\end{equation}
where $\bm\mu_{\bm{0}}(\bm\beta)=\bm\mu_{\bm Y|\bm U}(\bm 0,\bm\beta,\bm\theta)
...
\begin{equation}
  ...
  = \min_{\bm{u}}\left\|
        \begin{bmatrix}
          ...
          \bm{0}
        \end{bmatrix}
        - \begin{bmatrix}\bm M(\bm\beta,\bm\theta)\\\bm I_q\end{bmatrix}\bm{u}\right\|^2 .
  \end{aligned}
\end{equation}
...
\begin{equation}
  \label{eq:condlinsol}
  \left(\bm{M}(\bm\beta,\bm\theta)\trans
    \bm{M}(\bm\beta,\bm\theta)+\bm I_q\right)\tilde{\bm{u}}=
  \bm L(\bm\beta,\bm\theta)\bm L(\bm\beta,\bm\theta)\trans\tilde{\bm u}
  \bm{M}(\bm\beta,\bm\theta)\trans\left(\bm y-\bm\mu_{\bm{0}}(\bm\beta)\right)
\end{equation}
