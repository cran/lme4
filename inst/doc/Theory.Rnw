...
determine the linear predictor
\begin{equation}
  \label{eq:linpred}
...
\begin{equation}
  ...
  k(\sigma^2,\bm y)e^{-d(\bm\mu(\bm{X}\bm\beta+\bm{Z}\bm{b}),\bm{y})/(2\sigma^2)}=
  k(\sigma^2,\bm y)e^{-d\left(\bm\mu\left(\bm\eta_{\bm{b}}(\bm\beta,\bm{b})\right),
      \bm{y}\right)/\left(2\sigma^2\right)}
\end{equation}
In (\ref{eq:conddens}) the \emph{discrepancy function},
$d(\bm{\mu},\bm{y})$, defines the ``squared distance'' between the
\emph{conditional mean},
\begin{equation}
  \label{eq:conditionalmean}
  \bm\mu(\bm\eta)=\bm\mu\left(\bm\eta_{\bm{b}}(\bm\beta,\bm{b})\right)
  =\bm\mu\left(\bm{X}\bm\beta+\bm{Z}\bm{b}\right)
\end{equation}
and the observed data, $\bm{y}$, according to the form of the model.

In fact, for linear mixed models and for nonlinear mixed models, the
discrepancy is exactly the square of the usual Euclidean
distance, $d(\bm\mu,\bm y)=\left\|\bm y-\bm\mu\right\|^2$.
...
distribution completely determines the variance.  In such cases the
conditional density or conditional probability mass function can be
written $f_{\bm y|\bm b}(\bm y|\bm b,\bm\beta)=
k(\bm y)e^{-d(\bm\mu(\bm{X}\bm\beta+\bm{Z}\bm{b}),\bm{y})/2}$.

The normalization factor $k(\sigma^2,\bm y)$ depends only on the scale
factor, $\sigma^2$, if it is used in the model, and the observed
response, $\bm{y}$.
...
and the $q\times q$ symmetric, positive-semidefinite matrix
$\bm\Sigma(\bm\theta)$, called the \emph{relative variance-covariance
  matrix} of the random effects $\bm{b}$, is a function of a parameter
$\bm{\theta}$.  The condition that $\bm\Sigma$ is
...
Together $\bm{f}_i$ and $\bm{Z}_i$ determine the \emph{indicator
  interaction matrix} $\tilde{\bm{Z}}_i$ that is the horizontal
concatenation of the interaction of the columns of $\bm{Z}_i$ with the
matrix of indicators of the levels of $\bm{f}_i$.  That is, the
...
the indicator columns for $\bm{f}_i$.  The values of the nonzeros in
the $j$th vertical block in $\tilde{\bm{Z}}_i$ are the $j$th column of
$\bm{Z}_i$.

In the not-uncommon case that the linear model formula in a
random-effects term is `\code{1}' then $q_i=1$, $\bm{Z}_i$ is an
$m\times 1$ matrix all of whose elements are $1$ and
...
data frame with a ``subject'' factor could be constructed as
<<subj>>=
str(dat <- data.frame(subj = gl(5, 3, labels = LETTERS[1:5])))
@
if the data are ordered by subject.  A random effects term of the
...
@
and the indicator interaction matrix, $\tilde{\bm{Z}}_i$, which, in
this case, is simply the matrix of indicators,
<<extendedMM1,echo=FALSE>>=
with(dat, t(as(subj, "sparseMatrix")))
@

In the \package{lme4} package these sparse model matrices are stored
as compressed, column-oriented, sparse
matrices~\citep{davis06:csparse_book} of class \code{"dgCMatrix"}.
When such a matrix is printed, the systematic zeros are shown as `.'.

For a more general example, assume that each subject was observed at
times 1, 2 and 3.  If the \code{time} variable is
<<Z1prep>>=
dat$time <- rep(1:3, 5)
@
...
head(dat, n = 5)
@
(i.e. we have ordered the data first by subject then by time within
subject) then the first few rows of $\bm{Z}_1$ for a random-effects
term \code{(time|Subject)} are
<<Z1mat>>=
...
with $i$th diagonal block $\tilde{\bm T}_i=\bm T\otimes{\bm I}_{n_i}$
(respectively $\tilde{\bm S}_i=\bm S\otimes{\bm I}_{n_i}$).
...
Consider our example of 3 observations on each of 5 subjects.  For the
random effects term \code{(1|subj)} $q_i=1$ and $\bm{T}_i$, which is a
$1\times 1$ unit lower triangular matrix, must be $[1]$, the $1\times
1$ identity matrix. Hence $\tilde{\bm{T}}_i=\bm{I}_{5}$ and the
decomposition $\tilde{\bm\Sigma}=\tilde{\bm T}\tilde{\bm S}\tilde{\bm S}
\tilde{\bm T}\trans$ reduces to
$\tilde{\bm\Sigma}=\tilde{\bm S}\tilde{\bm S}$.
Furthermore, $\bm{S}_i=[\theta_{i,1}]$ subject to $\theta_{i,1}\ge 0$,
\begin{displaymath}
  \tilde{\bm S}_i=\theta_{i,1}\bm I_5
\end{displaymath}
and
\begin{displaymath}
  \tilde{\bm\Sigma}_i=\tilde{\bm S}_i\tilde{\bm S}_i=\theta_{i,1}^2\bm{I}_5 .
\end{displaymath}
...
performing the change of variable on an integral with respect to a
vector we must incorporate the determinant of the Jacobian of the
transformation.  For the transformation $\bm{b}=\bm{T}\bm{S}\bm{u}$
this is
\begin{displaymath}
...
\begin{equation}
  ...
  \begin{aligned}
    ...
    =&\int_{\bm{b}}\frac{k(\sigma^2,\bm y)}{\left(2\pi\sigma^2\right)^{q/2}|\bm{\Sigma}|^{1/2}}
    \exp\left[{\frac{d_{\bm{b}}(\bm{b},\bm\beta,\bm{y})+
          \bm{b}\trans\bm{\Sigma}^{-1}\bm{b}}{-2\sigma^2}}\right]\,d\bm{b}\\
    =&\int_{\bm{u}}\frac{k(\sigma^2,\bm y)}{\left(2\pi\sigma^2\right)^{q/2}}
...
\end{equation}
is called the \emph{penalized discrepancy}.  We will see that, for the
models we are considering, it is relatively straightforward to
...
solution of a penalized least squares problem.

For a fixed value of $\sigma^2$ the minimizer of the penalized
discrepancy maximizes the conditional density
\begin{displaymath}
...
\end{displaymath}
and $I$ becomes a multiple of the integral of the standard $q$-variate
normal density
\begin{equation}
  \label{eq:reducedint}
  ...
  =|\bm{L}|^{-1} .
\end{equation}
Expression (\ref{eq:reducedint}) requires that $\bm{L}$ is
non-singular, which will be the case when $\bm{L}$ is the Cholesky
factor of $\left.\frac{1}{2}\nabla_{\bm{u}}^2
  \delta(\bm{u}|\bm{\beta},\bm{\theta},\bm{y})\right|_{\bm{u}=\tilde{\bm{u}}}$.

Returning to expression (\ref{eq:likelihoodb}), we can now express the
Laplace approximation to the likelihood function or the log-likelihood,
\begin{equation}
  \label{eq:loglikelihood}
  \ell(\bm{\beta},\bm{\theta},\sigma^2|\bm{y})=
  \log L(\bm{\beta},\bm{\theta},\sigma^2|\bm{y}) ,
\end{equation}
which is more commonly used as the optimization criterion when
determining maximum likelihood estimates.  (Because the logarithm
function is monotonic, the maximizer of the log-likelihood function
also maximizes the likelihood function and generally the quadratic
approximation to the log-likelihood is much better than the quadratic
...
\begin{equation}
  \label{eq:LaplaceDev}
  -2\ell(\bm{\beta},\bm{\theta},\sigma^2|\bm{y})\approx-2\log[k(\sigma^2)]+
  \frac{\delta(\tilde{\bm u}|\bm\beta,\bm\theta,\bm y)}{\sigma^2}
  +2\log|\bm L| .
\end{equation}
Expression (\ref{eq:LaplaceDev}) will be an exact expression for the
log-likelihood and not just an approximation whenever the
...
(\ref{eq:conditionalMode}) to evaluate the log-likelihood using
(\ref{eq:LaplaceDev}), which is exact because the penalized
discrepancy is quadratic for linear mixed models.
...
$\bm{\theta}$ we evaluate the Cholesky factorization
\begin{equation}
  \label{eq:bigChol}
    \begin{bmatrix}
...
\begin{equation}
  ...
  \begin{aligned}
    ...
    &=r^2+\left\|\bm{L}_X(\bm\theta)\trans\left(\bm{\beta}-
...
\begin{align}
  ...
  \bm{L}_X(\bm\theta)\trans\widehat{\bm\beta}(\bm\theta)&=\bm{\ell}_{Xy}(\bm\theta)\\
  \label{eq:conditionalu}
  \bm{L}(\bm\theta)\trans\widehat{\bm{u}}(\bm{\theta})
  &=\bm{\ell}_{Vy}(\bm\theta)-\bm{L}_{VX}(\bm\theta)\trans\widehat{\bm\beta}(\bm\theta) .
\end{align}
...
of the fitted values to the observed data, $n\log r^2(\widehat{\bm\theta})$,
and a measure of model complexity, $2\log|\bm{L}(\widehat{\bm\theta})|$.
