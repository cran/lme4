...
\begin{figure}[tb]
  ...
  \caption{Travel time of sound waves in a sample of six railroad
    ...
    rail. The rail numbers are sorted by increasing mean travel time.}
...
for (i in seq(along = sg)) {
    .Call("lmer2_setPars", mm, sg[i], PACKAGE = "lme4")
    vals[i,] <- mm@deviance
}
...
print(xyplot(y ~ x | type, df, type = c("g", "l"),
             subset = !(type %in% c("REML", "ldX", "bqd", "Sdr")),
...
The model
<<sm1>>=
print(sm1 <- lmer2(Reaction ~ Days + (Days|Subject), sleepstudy))
@
...
print(image(sm1@A, colorkey = FALSE, aspect = 1),
...
<<sm1reconstruct>>=
str(dd <- diag(as(sm1@L, "sparseMatrix")))
c(ldZ = sum(log(dd[1:36]^2)), ldX = sum(log(dd[37:38]^2)), lr2 = log(dd[39]^2))
@
...
<<Om1>>=
print(Om1 <- lmer2(yield ~ nitro + Variety + (1|Block/Variety), Oats), corr = FALSE)
@
...
print(image(Om1@A, colorkey = FALSE, aspect = 1),
      split = c(1,1,2,1), more = TRUE)
print(image(Om1@L, colorkey = FALSE, aspect = 1),
      split = c(2,1,2,1))
@
\begin{figure}[tbp]
...
<<Om1a>>=
print(Om1a <- lmer2(yield ~ nitro + (1|Block/Variety), Oats), corr = FALSE)
@
...
print(image(Om1a@A, colorkey = FALSE, aspect = 1),
      split = c(1,1,2,1), more = TRUE)
print(image(Om1a@L, colorkey = FALSE, aspect = 1),
      split = c(2,1,2,1))
@
\begin{figure}[tbp]
...
<<sm2>>=
print(sm2 <- lmer2(Reaction ~ Days + (1|Subject) + (0+Days|Subject), sleepstudy), corr = FALSE)
@
...
print(image(sm2@A, colorkey = FALSE, aspect = 1),
      split = c(1,1,2,1), more = TRUE)
print(image(sm2@L, colorkey = FALSE, aspect = 1),
      split = c(2,1,2,1))
@
\begin{figure}[tbp]
...
<<Om2>>=
print(Om2 <- lmer2(yield ~ nitro + (1|Variety:Block) + (nitro|Block), Oats), corr = FALSE)
@
...
print(image(Om2@A, colorkey = FALSE, aspect = 1),
      split = c(1,1,2,1), more = TRUE)
...
<<Sm1>>=
print(Sm1 <- lmer2(attain ~ verbal * sex + (1|primary) + (1|second), ScotsSec), corr = FALSE)
@
and examine the \code{"isNested"} element of the \code{dims} slot.
<<Sm1dims>>=
Sm1@dims
@
...
<<Sm1fill>>=
c(A = length(Sm1@A@x), L = length(Sm1@L@x))
@
...
This quantity can be evalated as the sum of the deviance
